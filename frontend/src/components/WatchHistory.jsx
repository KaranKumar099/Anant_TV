...
const WatchHistory = () => {
  ...
  useEffect(()=>{
    const fetchHistory=async ()=>{
        try {
            ...
            const response=await axios.get("http://localhost:8000/api/v1/watch-history",
                {
                    headers:{
  ...
  const removeFromHistory = async (videoId) => {
    try {
      const token=localStorage.getItem("accessToken")
      const res=await axios.delete(`http://localhost:8000/api/v1/watch-history/${videoId}`,
        {
            headers:{
                Authorization: `Bearer ${token}`,
            },
  ...
  const clearAllHistory = async ()=>{
    try {
      const token=localStorage.getItem("accessToken")
      const res=await axios.delete(`http://localhost:8000/api/v1/watch-history`,
        {
            headers:{
                Authorization: `Bearer ${token}`,
            },
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex flex-col space-y-4">
          {history.slice().reverse().map((item) => (
            <div key={item._id} className="flex items-start gap-2 group hover:bg-gray-300 transition-colors duration-400 cursor-pointer">
              <div className="relative w-60 h-36 flex-shrink-0">
                <img src={item.video?.thumbnail} alt={item.video?.title} className="w-full h-full object-cover rounded"/>
              ...
              <div className="flex flex-col flex-1 p-1">
                ...
                <p className="text-sm text-gray-600">{item.video?.owner.username} • {item.video?.views} views</p>
                <p className="text-xs text-gray-600">{item.video?.description}</p>
              </div>

              <button onClick={() => removeFromHistory(item._id)}
