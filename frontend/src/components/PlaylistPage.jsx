...
const PlaylistPage = () => {
    ...
    const removeVideoHandler=async (videoId)=>{
      try {
        const token=localStorage.getItem("accessToken")
        const response=await axios.patch(`http://localhost:8000/api/v1/playlist/remove/${videoId}/${playlistId}`,
            {playlistId, videoId},
            {
                headers:{
                    Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-4">
      <div className="lg:w-1/3 bg-gradient-to-b from-gray-800 to-yellow-800 rounded-lg overflow-hidden">
        <img src={videos[0]?.thumbnail} alt="Playlist Thumbnail" className="w-full h-90 object-cover p-4"/>
        <div className="p-4 text-white">
          ...
          <div className="mt-4 flex gap-3">
            ...
            <button className="bg-gray-700 px-3 py-2 rounded">add</button>
      ...
      <div className="flex-1">
        {videos.map((video) => (
          <div key={video._id} className="main relative flex gap-3 mb-2 border-b py-1 hover:bg-gray-300 transition-colors duration-400 cursor-pointer">
            ...
            <button className="cursor-pointer rounded-full h-8 w-11 hover:bg-gray-300 transition-colors duration-500"
            ...
            <div className="more-options-modal absolute right-5 top-8 z-10 flex-col items-start bg-gray-300 w-[14rem] rounded-lg overflow-hidden hidden">
              <div className="w-full py-1 px-3 hover:bg-gray-400 transition-colors duration-300" onClick={()=>removeVideoHandler(video._id)}>
                <button className="cursor-pointer">Remove from Playlist</button>
              </div>
              <div className="w-full py-1 px-3 hover:bg-gray-400 transition-colors duration-300">
                <button className="cursor-pointer">Move to Top</button>
              </div>
              <div className="w-full py-1 px-3 hover:bg-gray-400 transition-colors duration-300">
                <button className="cursor-pointer">Move to Bottom</button>
              </div>
            </div>
          </div>
      ))}
