  ...
  useEffect(()=>{
    async function fetchSubscriptions () {
      ...
      try{
        ...
        const res=await axios.get(`http://localhost:8000/api/v1/subscriptions/u/${user._id}`,
          {
            headers:{
      ...
      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {subscriptions.map((channel) => (
          ...
          >
            ...
            <button className="mt-4 block w-full bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 
              text-sm py-1 rounded" onClick={()=>navigateToChannelDashboard(channel.channel)}>
              View Channel
            </button>
          </div>
        ))}
