...
function AddToPlaylist({videoId}) {
    ...
    useEffect(()=>{
        ...
        const fetchPlaylists=async ()=>{
            try {
                ...
                const response=await axios.get(`http://localhost:8000/api/v1/playlist/user/${userId}`,
                    {
                        headers:{
  ...
  const addVideoToPlaylistAction = async (playlistId) => {
      ...
      try {
        const token = localStorage.getItem("accessToken")
        const res = await axios.patch(`http://localhost:8000/api/v1/playlist/add/${videoId}/${playlistId}`,
          {videoId, playlistId},
          {
              headers:{
                  Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const selected = playlists.filter((item) => checked[item._id]).map((item) => item._id);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-800">Save video to...</h1>
        <button className="h-6 w-6 bg-amber-200" onClick={handleExit}><i className="ri-close-large-line"></i></button>
      </div>
      <div className="mt-6 space-y-3">
        {playlists.map((item) => (
          <label key={item._id} htmlFor={item._id} className="flex items-center gap-3 select-none rounded-xl 
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <button type="submit" className="inline-flex items-center justify-center rounded-2xl px-5 py-2.5 text-sm font-medium
