...
function YourChannel() {
  ...
  useEffect(()=>{
    ...
    const fetchChannelDets= async ()=>{
      try {
          const stats=await axios.get(`http://localhost:8000/api/v1/dashboard/${id}/stats`,
              {
                  headers:{
    ...
    const fetchPlaylists=async ()=>{
      try {
          ...
          const response=await axios.get(`http://localhost:8000/api/v1/playlist/user/${id}`,
              {
                  headers:{
                      Authorization: `Bearer ${token}`,
                  },
      ...
      {user && channelDets ? 
        (<>
          <div className="relative w-full h-56 p-4 pb-0 overflow-hidden group">
            <img src={user?.coverImage} alt="Channel Banner"
            ...
            <p className="absolute top-1/2 left-1/2 text-center -translate-1/2 opacity-0 group-hover:opacity-100">
                <label htmlFor="coverFile" className="cursor-pointer">
                  <i className="ri-camera-line text-2xl"></i><br></br>
                    Click to Change
                </label>
                <input type="file" name="coverFile" id="coverFile" onChange={handleChangeImages} className="hidden"/>
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center p-4 gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 text-gray-700">description 2 to 3 line</p>
              <a href="#"
              ...
              <div className="flex gap-3 mt-3">
                ...
                <button className="px-4 py-2 bg-gray-200 rounded-md cursor-pointer hover:bg-gray-300 transition-colors duration-500" 
            ...
            <div className="flex-1 text-center bg-gray-300 py-10">
              <h1 className="text-6xl">{channelDets.subsCount}</h1>
              <p>Subscribers</p>
            </div>
            <div className="flex-1 text-center bg-gray-300 py-10">
          ...
          <div className="flex border-b border-gray-200 mt-4 px-4 overflow-x-auto">
            <button className={`px-4 py-2 text-sm font-medium ${ 
              ...
              onClick={(e)=>setCurrent(e.target.name)}>Videos</button>
            <button className={`px-4 py-2 text-sm font-medium ${ 
              current === "playlists"
              ? "text-black border-b-2 border-black"
              : "text-gray-600 hover:text-black"}`
            } name="playlists" 
                onClick={(e)=>setCurrent(e.target.name)}>Playlists</button>
          </div>
          
          <div className="flex gap-x-4 gap-y-8 flex-wrap p-4">
            { current === "videos"
                ? videos?.map((item) => (
                    <VideoCard key={item._id} title={item.title} thumbnail={item.thumbnail} views={item.views} uploaded={item.createdAt.substr(0,10)} 
                        buttons={false} duration={item.duration} videoId={item._id}/>
                  ))
                : playlists?.map((item) => (
                    <PlaylistCard key={item._id} thumbnail={item?.videos?.[0]?.thumbnail} playlistName={item.name} 
                        vdoCount={item.videos?.length} updated={item.updatedAt.substr(0,10)} playlistId={item._id}/>
                  ))
            }
          </div>
