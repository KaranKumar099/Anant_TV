...
function EditPlaylist() {
    ...
    useEffect(()=>{
        async function prevDets(){
          ...
          const res= await axios.get(`http://localhost:8000/api/v1/playlist/${id}`,{
              headers: {
  ...
  const handleFormSubmit=async (e)=>{
    ...
    try {
        const token= localStorage.getItem("accessToken")
        const response = await axios.patch(`http://localhost:8000/api/v1/playlist/${id}`, 
          { 
  ...
  return (
    <div className='h-full w-full flex-1 flex flex-col items-center'>
        ...
        <form className='flex flex-col gap-4' onSubmit={handleFormSubmit}>
            <div>
                <label>Name</label><br />
                <input name="name" type="text" required value={playlistDets.name} onChange={handleChange}
                    className='py-1 px-2 w-md outline mt-1' />
            </div>
            <div>
                <label>Description</label><br />
                <textarea name='description' rows="4" value={playlistDets.description} onChange={handleChange}
                    className='py-1 px-2 w-md outline mt-1'></textarea>  
            </div>
            <button type="submit" className='p-2 cursor-pointer text-white px-4 py-2 rounded bg-blue-500 transition'>Edit</button>
        </form>
    </div>
  )
}
